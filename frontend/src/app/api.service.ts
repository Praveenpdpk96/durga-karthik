...
export interface LlmAnalysisResponse { engine:string; model:string; aiEnhanced:boolean; baselineScore:number; fitAssessment:string; seniorityAssessment:string; domainAssessment:string; strengths:string[]; gaps:string[]; resumeImprovements:string[]; interviewFocus:string[]; note:string; }
...
@Injectable({providedIn:'root'}) export class ApiService {
 ...
 aiMatch(resumeText:string,jobDescription:string):Observable<LlmAnalysisResponse>{return this.http.post<LlmAnalysisResponse>(`${this.baseUrl}/matches/ai`,{resumeText,jobDescription});}
